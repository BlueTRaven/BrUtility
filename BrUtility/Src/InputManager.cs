///taken from https://github.com/A1rPun/MonoGame.InputManager
using System;
...
namespace A1r.Input
{
	...
	public class InputManager : GameComponent
	{
		...
		private List<int> gamepadIndices;
		...
		public bool IsHeld(object leftControl)
		{
		...
		#region IsRepeated
		private const int repeatedTime = 30;

		public bool IsRepeated(Keys key)
		{
			bool held = IsHeld(key);

			return CheckRepeatedHost(held, key.ToString());
		}
		public bool IsRepeated(MouseInput input)
		{
			bool held = IsHeld(input);

			return CheckRepeatedHost(held, input.ToString());
		}
		public bool IsRepeated(Buttons button, int index)
		{
			bool held = IsHeld(button);

			return CheckRepeatedHost(held, button.ToString());
		}
		public bool IsRepeated(Input input)
		{
			bool held = IsHeld(input);

			return CheckRepeatedHost(held, input.ToString());
		}
		public bool IsRepeated(Input input, int index)
		{
			bool held = IsHeld(input);
		...
		private bool CheckRepeatedHost(bool held, string key)
		{
			if (held)
			{
				if (repeatedKeys.ContainsKey(key))
					repeatedKeys[key]++;
				else repeatedKeys.Add(key, 0);
			}
			else
			{
				if (repeatedKeys.ContainsKey(key))
			...
			return repeatedKeys.ContainsKey(key) && repeatedKeys[key] > repeatedTime;
		}
		#endregion
		...
		public bool FindNewGamepads()
		{
			...
			var a = GamePad.GetState(PlayerIndex.One);

			List<GamePadState> gpses = new List<GamePadState>();
			for (int i = 0; i < GamePad.MaximumGamePadCount; i++)
				gpses.Add(GamePad.GetState(i));
